...
//  LINK_TO_VE8 - Тот в паре, кто подключен к 1986ВЕ8Т - пропускается ожидание Autonegotiation или Link в линии
  ...
  //#define IS_STARTER  
  #define VE8_STARTER_FIX   //  Не стартуер обмен с первого фрейма, 1986ВЕ1 принимает только второй
#endif
...
#else
      FrameLen = StatusRX.Fields.Length - FRAME_CRC_SIZE;  
#endif
...
#if !defined(LINK_TO_VE8) && !defined(USE_LOOP_BACK) && !defined(USE_BOARD_VE_8)
  //  Wait
  #ifdef USE_AUTONEG  
...
uint32_t CheckFrameData(void)
{
  ...
  dataCount = (uint32_t) ((ptr8[1]) | (ptr8[0] << 8));
...
void FillFrameTXData(uint32_t frameIndex, uint32_t frameLength)
{
  ...
  ptrDataTX[dataCount - 1] = (uint8_t)(frameLength & 0xFF);
}
